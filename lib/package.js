...
Package.prototype.buildReport = function(masterReport, entryModule, moduleOptions)
{
	...
	{
	    // See if we have any resolvers that map to other packages.
	    if (typeof self.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].resolvers !== "undefined")
	    {
	        var done = Q.ref();

	        UTIL.forEach(self.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].resolvers, function(resolverInfo)
	        {
	            resolverInfo[1].forEach(function(resolver)
	            {
                    if (!/^[\.\/]/.test(resolver))
	                {
                        done = Q.when(done, function()
                        {
                            ...
                            if (!self.descriptor.mappings) {
                                throw new Error("Cloud not resolve dynamic load resolver '" + resolver + "' against package '" + self.path + "'. No 'mappings' property found in package descriptor!");
                            }
                            if (!self.descriptor.mappings[resolverParts[0]]) {
                                throw new Error("Cloud not resolve dynamic load resolver '" + resolver + "' against package '" + self.path + "'. No 'mappings[\"" + resolverParts[0] + "\"]' property found in package descriptor!");
                            }
                            
                            var pkgPath = FS.realpathSync(self.path + "/" + self.descriptor.mappings[resolverParts[0]]);                            
                            
                            if (typeof masterReport.packages[self.path].mappings[resolverParts[0]] === "undefined")
                            {
                                masterReport.packages[self.path].mappings[resolverParts[0]] = pkgPath;
                            }
                            ...
                            // Override bundler config for package if we have config info set for it in our package.
                            if (typeof self.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].packages === "object" &&
                                typeof self.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].packages[resolverParts[0]] === "object" &&
                                typeof self.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].packages[resolverParts[0]].descriptor === "object")
                            {
                                options.overrideDescriptor = self.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].packages[resolverParts[0]].descriptor;
                            }

                            return self.options.bundler.getInitializedPackageForPath(pkgPath, options).then(function(pkg)
                            {
                                pkg.buildReport(masterReport, null);
                            });
                        });
	                }
	            });

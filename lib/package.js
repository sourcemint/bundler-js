...
Package.prototype.init = function(options)
{
	...
	// Try and load package descriptor.
	this.descriptor = UTIL.deepCopy(this.options.descriptor || {});
	...
	if (typeof this.descriptor.directories.lib === "undefined") {
		this.descriptor.directories.lib = "lib";
	}
...
Package.prototype.buildReport = function(masterReport, entryModule)
{
	...
	function followModule(uri)
	{
		...
		{
			return bundlerAdapter.parseModule(modulePath).then(function(moduleReport)
			{
					...
					UTIL.forEach(moduleReport.staticLinks, function(linkId)
					{
						done = Q.when(done, function()
						{
							...
							if (/^\./.test(linkId[0]))
							{
								...
								return followModule(PATH.dirname(modulePath) + "/" + linkId[0]);
							}
							else
							{
								...
								if (typeof self.descriptor.mappings !== "object") {
									throw new Error("Package descriptor '" + self.path + "/package.json' does not specify 'config.mappings' needed to resolve aliased module '" + linkId[0] + "'.");
								}

								var linkIdParts = linkId[0].match(/^([^\/]*)\/(.*?)$/).slice(1);

								if (typeof self.descriptor.mappings[linkIdParts[0]] === "undefined") {
									throw new Error("Package descriptor '" + self.path + "/package.json' does not specify 'config.mappings[\"" + linkIdParts[0] + "\"]' needed to resolve aliased module '" + linkId[0] + "'.");
								}
								...
								// NOTE: We inherit the bundler configuration in case our dependency packages are of the same
								//       type but do not declare bundler config.
								
								return self.options.bundler.getInitializedPackageForPath(masterReport.packages[self.path].mappings[linkIdParts[0]], {
									descriptor: {
										config: {
								...
								{
									return pkg.buildReport(masterReport, linkIdParts[1]);
								});
							}
						});
					});
...
Package.prototype.loadAdapterImplFor = function(uri, name)
{
	...
	// NOTE: This assumes a local development path.
	// TODO: Replace with `require.resolve("sourcemint-sdk-requirejs")`
	var adapterPath = __dirname + "/../../../platform-nodejs/0";
	if (uri === "github.com/sourcemint/sdk-requirejs/0")
	{
		adapterPath = __dirname + "/../../../sdk-requirejs/0";
	}

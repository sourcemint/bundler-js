    	    ...
    	    function init2()
    	    {
                return self.getInitializedPackageForPath(self.packagePath, self.options).then(function(pkg) {
	                ...
	                if (self.options.bundleLoader === true) {
						...
						var bundleAdapter = pkg.loadAdapterImplFor(pkg.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].adapter, "bundler");

	                    self.bundle.setBundleLoader(self.options.bundleLoader, {
	                        bundleUrlPrefix: self.options.bundleUrlPrefix,
	                        platformLoaderSource: bundleAdapter.getPlatformLoaderSource()
	                    });
	                }

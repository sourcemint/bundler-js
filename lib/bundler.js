	...
	return Q.when(self.programPackage(), function(pkg)
	{
		return self.generateReport().then(function(sourceReport)
		{
			...
			return Q.when((typeof bundlerAdapter.remapSources === "function")?bundlerAdapter.remapSources(sourceReport):sourceReport).then(function(mappedReport)
			{
				...
				{
				    // Write bundle header that will be replaced below.
                    bundleStream.write('//                                                                                                                                                                                    \n');
			...
			{
                // Re-open the bundle and inject the bundle report.
	            // TODO: Use bundle FS abstraction here.

			    var deferred = Q.defer();
			    FS.stat(self.distributionPath + ".js", function(err, stats)
			    {
                    Q.call(function() {
                        if (err) throw err;
        			    FS.open(self.distributionPath + ".js", "a", function(err, fd)
        			    {
        			        Q.call(function() {
            			        if (err) throw err;
            			        
            			        var report = new Buffer("// @sourcemint-bundle-report: " + JSON.stringify(masterReport) + "\n");

            			        var header = new Buffer("// @sourcemint-bundle-partition-map: " + JSON.stringify({
            			            report: [stats.size, stats.size + report.length]
            			        }));

            			        FS.write(fd, header, 0, header.length, 0, function(err, written, buffer)
            			        {
            			            // TODO: Check err for error.
                                    FS.write(fd, report, 0, report.length, stats.size, function(err, written, buffer)
                                    {
                                        // TODO: Check err for error.

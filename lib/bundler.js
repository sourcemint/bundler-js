					...
					if (typeof resolver === "string")
					{
						...
						}, function (err, files)
						{
							...
							else
							{
								...
								{
									var modulePath = FS.realpathSync(context.pkg[0] + "/" + file);

									dynamicLoadBundles[modulePath] = {
										uri: file,
										packagePath: context.pkg[0],
										modulePath: modulePath,
										pkgNamespace: context.pkgNamespace
									};
								});
								deferred.resolve();
							}
									...
									files.forEach(function(file)
									{
										if (FS.statSync(pkg[0] + "/" + file).isFile())
										{
											done = Q.when(done, function()
											{
												...
												var newFile = FS.createWriteStream(self.distributionPath + "/" + file);

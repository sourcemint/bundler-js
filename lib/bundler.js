...
var Bundler = function(packagePath, distributionPath, options)
{
	...
	if (this.options.forceCompleteBuild === true)
	{
	    if (PATH.existsSync(this.distributionPath)) {
	        WRENCH.rmdirSyncRecursive(this.distributionPath);
	    }
        if (PATH.existsSync(this.distributionPath + ".js")) {
            FS.unlinkSync(this.distributionPath + ".js");
        }
        if (PATH.existsSync(this.distributionPath + ".manifest.json")) {
            FS.unlinkSync(this.distributionPath + ".manifest.json");
        }
	}

    if (this.options.rebuildChanges === true)
    {
        // We need the manifest to track changes.
        this.options.writeManifest = true;
    }

	// Give back a promise for the package until it has been provisioned locally.
	var deferred = Q.defer();
	this.getInitializedPackageForPath(this.packagePath, this.options).then(function(pkg) {
...
Bundler.prototype.generateBundles = function()
{
	...
	function bundleModule(module, context)
	{
		...
		if (treatAs === "json")
		{
            ...
            context.bundleStream.write('    require.memoize("' + canonicalId + '", ');
        ...
        if (treatAs === "text")
        {
            var code = FS.readFileSync(module[0]).toString();
            context.bundleStream.write('    require.memoize("' + canonicalId + '", ');
            context.bundleStream.write(('"' + encodeURIComponent(code) + '"'));
            context.bundleStream.write(');\n');
            return;
        } else
		...
		}).then(function()
		{
			...
			var resolvers = context.pkgObj.descriptor.config["github.com/sourcemint/bundler-js/0/-meta/config/0"].resolvers[localId];
				...
				done = Q.when(done, function()
				{
					if (typeof resolver === "string")
					{
						if (/^[\.\/]/.test(resolver))
                        {
                            ...
                            return resolve(resolver, context.pkg[0], context.pkgNamespace);
                        }
                        else
                        {
                            // Resolve a link relative to a mapped package.
                            // TODO: Use a more generic resolver here that is used in other places as well.
                            var resolverParts = resolver.split("/");
                            var pkgPath = FS.realpathSync(context.pkgObj.path + "/" + context.pkgObj.descriptor.mappings[resolverParts[0]]);
                            
                            return self.getInitializedPackageForPath(pkgPath).then(function(pkg)
                            {
                                var resolver = resolverParts.slice(1).join("/");
                                // Prefix lib path if applicable.
                                if (pkg.descriptor.directories.lib !== "") {
                                    resolver = pkg.descriptor.directories.lib + "/" + resolver;
                                }
                                    
                                if (PATH.extname(resolver) === "") {
                                    resolver += ".js";
                                }

                                return resolve(resolver, pkgPath, packageIdForPath(pkgPath));
                            });
                        }

                        function resolve(resolver, packagePath, pkgNamespace)
                        {
                            var deferred = Q.defer();
	...
	return Q.when(self.programPackage(), function(pkg)
	{
		return self.generateReport().then(function(sourceReport)
		{
			...
			return Q.when((typeof bundlerAdapter.remapSources === "function")?bundlerAdapter.remapSources(sourceReport):sourceReport).then(function(mappedReport)
			{
                ...
                bundleStream.once("error", function(err) {
                    deferred.reject(err);
                });
				bundleStream.once("open", function() {
			        deferred.resolve();
				});
				return Q.when(deferred.promise).then(function()
				{
    				...
    				return Q.when(done, function()
    				{
    					...
    					bundleStream.once("close", function()
    					{
    						deferred.resolve(masterReport);
    					});
    					bundleStream.end();
			...
			}).then(function()
			{
				...
				UTIL.forEach(dynamicLoadBundles, function(dynamicLoadBundle)
				{
					done = Q.when(done, function()
					{
						...
						return ((new Bundler(dynamicLoadBundle[1].packagePath || self.packagePath, distPath, options)).generateBundles());
					});
				});
			...
			{
			    if (self.options.writeManifest === true)
			    {
    			    var manifestPath = self.distributionPath + ".manifest.json";
    			    FS.writeFileSync(manifestPath, JSON.stringify(masterReport));
			    }
			});
		});
	});
